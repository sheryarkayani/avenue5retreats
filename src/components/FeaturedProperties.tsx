...
export default function FeaturedProperties() {
  ...
  return (
    <section ref={ref} className="w-full py-16 md:py-24 lg:py-32 bg-gradient-to-b from-[#002F5F] to-[#001F3F] text-white overflow-hidden relative">
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 0.1 }}
      ...
      <div className="container px-4 md:px-6 max-w-7xl mx-auto relative z-10">
        ...
        <Carousel className="w-full max-w-6xl mx-auto">
          <CarouselContent className="-ml-2 md:-ml-4">
            <AnimatePresence mode="wait">
              {properties.map((property, index) => (
                <CarouselItem key={property.id} className="pl-2 md:pl-4 md:basis-1/2 lg:basis-1/2">
                  <motion.div
          ...
          <CarouselPrevious className="left-4 bg-[#D4AF37] text-[#001F3F] hover:bg-[#C4A137] transition-all duration-300 hover:scale-110" />
          <CarouselNext className="right-4 bg-[#D4AF37] text-[#001F3F] hover:bg-[#C4A137] transition-all duration-300 hover:scale-110" />
        </Carousel>
        <motion.div 
